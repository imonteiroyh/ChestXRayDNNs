...
class DenseNet201UNet(nn.Module):
    def __init__(self, n_inputs=1, device="cuda"):
        ...
        self.network.features.conv0 = nn.Conv2d(n_inputs, 64, kernel_size=7, stride=2, padding=3, bias=False)

        self.network.features.relu0.register_forward_hook(self.get_features(4))
        self.network.features.transition1.conv.register_forward_hook(self.get_features(3))
        self.network.features.transition2.conv.register_forward_hook(self.get_features(2))
        self.network.features.transition3.conv.register_forward_hook(self.get_features(1))
        self.network.features.register_forward_hook(self.get_features(0))

        self.relu = nn.ReLU()

        self.conv1_1 = nn.Conv2d(2816, 256, kernel_size=3, padding="same")
...
if __name__ == "__main__":
    ...
    model = DenseNet201UNet(device="cpu")
